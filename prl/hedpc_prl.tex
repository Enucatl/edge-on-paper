...
%Furthermore, depending on the sensitivity of the phase sensitive technique, an enhanced contrast-to-noise ratio (CNR) in phase images compared to absorption images may apply for a variety of materials or in different tissues.


% Similar to attenuation, an energy dependent parameter can be derived, describing the phase shift properties of a material.
% 
% In a material, the X-ray phase can be shifted with respect to the undisturbed wave, which is, compared to attenuation, a fundamentally different interaction mechanism and can mainly be attributed to the coherent (Rayleigh) scattering of X-rays in a material.
% 
% 
% Regarding the wave properties, a transition from one medium into another causes a change in the wave's phase velocity and thus in a net change of the output phase (a phase shift) behind the object. Since no detection device is able to measure a phase shift directly, advanced techniques are required to determine the phase shift in a material.
% 
% Referring to the particle properties of X-rays, photons can be randomly scattered in the material. Compton scattering 
% 
% Depending on the imaging system (geometry and energy), wide angle and small angle scattering can be sources of noise in conventional X-ray images. For the special type of scattering which occurs exclusively in forward direction and under ultra small angles (order of nano radiants), the particles usually remain within the area of a detector pixel. This effect is not recorded by absorption images and cannot directly be detected with standard techniques.
% 
