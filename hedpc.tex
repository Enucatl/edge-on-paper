...
index of refraction $n=1 - \delta + i \beta$. The imaginary part $\beta$ is
...
Phase-sensitive imaging is a desirable modality, as it delivers a
...
atomic number~\cite{Qi2010}. An enhanced \ac{CNR} in images compared to
...
high spatial and temporal coherence, which is available only at synchrotron
...
addition to phase sensitivity, analyzer-based and Talbot interferometry
approaches also provide (with different retrieval mechanisms) information
...
removes one on the major hurdle which prevented grating interferometry to be
applied at very high X-ray energies so far, namely the fabrication of
gratings with high-aspect ratios. The aspect ratio, given by
\begin{equation} \text{R} = \frac{2h}{p}, \end{equation} where $p$ is the
grating period and $h$ the  structure height, is normally limited by the
fabrication process, usually photolithography~\cite{David2002} or X-ray
lithography~\cite{Mohr2012} as grating structures tend to collapse or deform
...
necessary for a reasonable length of the experimental arrangement, it would
...
efficiently blocked by the gratings, which requires even higher aspect
...
TT and AM wrote the manuscript text and performed the experiments. All
