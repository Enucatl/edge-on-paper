...
\mu(\mathbf{r},\lambda) \d{} \mathbf{r}$. Similarly, the real part
...
\int_0^d \phi(\mathbf{r}) \d{} \mathbf{r}$.
...
that is proportional to the scatter strength of details in the sample\cite{Pfeiffer2008}.
...
\begin{figure} [ht]
    ...
    \caption{Grating design mask for
        the edge-on illumination approach and scanning electron microscope
        ...
        The interrupting bridges used for stabilizing the grating
        in the lithographic process can be seen with the SEM\cite{Kenntner2010}.}
...
line \cite{Weitkamp2005} and an exposure time of 15 seconds per step. The long
