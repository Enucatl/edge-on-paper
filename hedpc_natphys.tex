...
Phase-sensitive
imaging is a desirable modality, as it can provide a complementary source of
...
The vast majority of phase-sensitive techniques, including crystal analyzer based
...
spatial coherence but are available on microfocus sources.
...
apertures \cite{Munro2012} and Talbot-Lau interferometry \cite{Pfeiffer2006}.

WHAT IS TLINTERFEROMETER

In addition to phase sensitivity, these techniques provide a dark-field
contrast, also called scatter contrast or visibility-reduction contrast,
that is proportional to the integrated local small angle scattering power
from microscopic density fluctuations in a specimen\cite{Pfeiffer2008}.
...
compatible with the entire diagnostic energy range of X-rays, 
compact imaging arrangements and 
...
The maximum achievable aspect ratios of current fabrication techniques
\cite{David2007,Kenntner2010} are around 60.
...
layer, resuluting in lower absorption contrast. Due to the weaker energy
...
would be opaque at lower energies. Finally, the approach is not
limited to X-ray imaging, but could be easily applied to other grating-based
...
gratings (\SI{5}{\percent}). The exposure time can be shortened with better
gratings and an efficient detector.
